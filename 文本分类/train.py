...
def CNN_net(data,dict_dim, class_dim=33, emb_dim=128, hid_dim=128,hid_dim2=98):
...
paddle.enable_static() # paddle 2.x 版本需加上
...
# 获取训练数据读取器和测试数据读取器
train_reader = paddle.batch(reader=train_reader('./datasets/train_list.txt'), batch_size=128)
test_reader = paddle.batch(reader=test_reader('./datasets/test_list.txt'), batch_size=128)
